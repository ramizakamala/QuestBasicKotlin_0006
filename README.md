...
- Setiap file bersifat mandiri (self-contained) dan punya `fun main()` sendiri,
  sehingga bisa dijalankan satu per satu di Kotlin Playground.
- Kode tiap sub bab mengikuti materi yang diberikan.
- Baris yang di materi sengaja ditunjukkan sebagai error kompilasi (misalnya mengisi `null`
  ke variabel non-nullable pada sub bab Null safety) ditulis sebagai komentar supaya
  program tetap bisa dijalankan.
- Tautan playground seluruh sub bab tanpa screenshot juga tersedia di
  [`playground-links.txt`](playground-links.txt).
